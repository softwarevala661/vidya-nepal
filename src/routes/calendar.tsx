...
export const Route = createFileRoute("/calendar")({
  head: () => ({
    meta: [
      { title: "School Calendar | Vidya ERP" },
      { name: "description", content: "Full Bikram Sambat calendar with events, exams and public holidays." },
      { property: "og:title", content: "School Calendar — Vidya ERP" },
      { property: "og:description", content: "Full Bikram Sambat calendar with events, exams and public holidays." },
    ],
  }),
  component: Page,
});
...
function Page() {
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader
        emoji="🗓️"
        title={bi("पात्रो", "School Calendar")}
        subtitle="Full Bikram Sambat calendar with events, exams and public holidays."
        actions={<LoadingSwitch loading={loading} onChange={setLoading} />}
      />

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard emoji="🗓️" label={bi("कुल", "Total records")} value={1024} tone="primary" />
